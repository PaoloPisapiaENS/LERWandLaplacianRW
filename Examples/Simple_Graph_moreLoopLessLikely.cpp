///////////////////////////////////////////////////////////////
// We want to prove the statement: less loops = more likely. //
// For this to be true, all weights must be the same         //
///////////////////////////////////////////////////////////////

// Include a header file containing all the necessary libraries and function prototypes
#include "/mnt/d/Offline_Documents/University/PhD_Paris/PhD_work/Simulations/Simple_Graph/AuxiliaryCodes/my_header.h" 

#define SESSION 0  // To do slightly different things: 0: directed graph; 2: undirected graph;
...
int main()
{
  ...
  g.add_edge(0, 4, q, ud);      // Absorption

  g.add_edge(1, 1, beta1, ud);
  g.add_edge(1, 3, beta3, ud);
  g.add_edge(1, 4, q, ud);      // Absorption

  g.add_edge(2, 3, beta1, ud);
  g.add_edge(2, 4, 2*q, ud);      // Absorption
#endif 
...
void RandomWalk( Graph g)
{
  ...
  while(nedges !=0) // The loop stops when we can no longer move from a vertex, i.e. when its not connected to any other vertex
  {
    ...
    if (j==3)     // i.e. if it reaches the destination
    {
